...
uses
  ...
  Menus, ComCtrls, ClassFaktury, StdCtrls, Grids, Typy, ClassTlac;
...
uses DialogDodaciKFakture, FormText;
...
begin
  ...
  try
    Faktury := TFaktury.Create( 'Fakodb.dbf' , 'Fotext.dbf' , 'Adresy.dbf' );
  except
    else MessageDlg( 'Datab·zovÈ s˙bory s˙ pouûÌvanÈ in˝m programom' , mtError , [mbOk] , 0 );
  end;
...
var I : integer;
    F : TextFile;
begin
  if Tlac <> nil then Tlac.Free;
  if Faktury <> nil then Faktury.Free;
