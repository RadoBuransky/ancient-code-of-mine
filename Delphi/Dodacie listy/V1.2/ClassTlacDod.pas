...
var TlacDod : TTlacDod;

implementation

uses FormMoznosti, Printers;
...
procedure TTlacDod.Vytlac( iDodaci : PDodaci );
begin
  PocetStran := 0;
  PocetPoloziek := 0;
  PocetSeriovych := 0;
  ...
  Spracuj( FormMozn.SpolocnyText , SpolocnyText );
...
procedure TTlacDod.VytlacStranku;
var S : string;
    I, J, Max : integer;
    ...
    begin
      if PocetSeriovych = 0 then
        begin
          ...
          if TPolozka( TFaktura( Dodaci.Faktura^ ).Polozky[I]^ ).Zaruka <> '' then
...
procedure TTlacDod.Spracuj( iStringList : TStringList; var oSpolocnyText : TStringList );
var I, J : integer;
...
begin
  oSpolocnyText.Clear;
  if iStringList.Count = 0 then exit;
