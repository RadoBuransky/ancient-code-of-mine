...
begin
  ...
  if (FSaved) then
    StatusBar.Panels[0].Text := 'UloûenÈ'
  else
    StatusBar.Panels[0].Text := 'NeuloûenÈ';
...
begin
  ...
  case (ActivePage) of
    pDodaci : begin
                ...
                begin
                  MessageDlg( 'S˙bor sa ned· uloûiù!' , mtError , [mbOK] , 0 );
                  exit;
                end;
              end;
  end;
...
begin
  ...
  case (ActivePage) of
    pDodaci : begin
                ...
                SaveDialog.Filter     := 'DodacÌ list|*.'+Globals.DodacieExt;
                SaveDialog.InitialDir := Globals.DodacieDir;
                SaveDialog.Title      := 'Uloûiù ako';
...
var Result : word;
    Dir, Name, Ext : string;
begin
  OpenDialog.Filter     := 'DodacÌ list|*.'+Globals.DodacieExt;
  ...
  begin
    if (not Saved) then
    begin
      Result := MessageDlg( 'Dokument nie je uloûen˝! Chcete ho uloûiù?' , mtError , [mbYES,mbNO,mbCancel] , 0 );
    ...
    begin
      ...
      begin
        MessageDlg( 'S˙bor sa ned· otvoriù!' , mtError , [mbOK] , 0 );
        exit;
      end;
